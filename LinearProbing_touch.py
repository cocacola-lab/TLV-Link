...
from model.LinearModel import LinearClassifierResNet
from data.dataset import TouchFolderLabel,FeelingLabel

from transformers import AutoConfig
import copy
from model.process_clip import convert_model_to_lora
...
def parse_option():
    ...
    # model definition
    parser.add_argument('--model', type=str, default='tlv')
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='touch_and_go', choices=['touch_and_go', 'touch_hard', 'touch_rough', 'feeling'])
    ...
    # tlv-link
    parser.add_argument("--clip-type", type=str, default="cl", choices=['tv', 'tl', 'vl', 'tlv', 'cl', 'vl_new'], help="",)
    parser.add_argument("--num_frames", type=int, default=8, help="",)
    parser.add_argument("--tube_size", type=int, default=1, help="",)
    parser.add_argument("--add_time_attn", default=False, action="store_true", help="")
    ...
    parser.add_argument('--force_patch_dropout', type=float, default=0.5, help='')
...
def get_train_val_loader(args):
    ...
    if args.dataset == 'touch_and_go' or args.dataset == 'touch_rough' or args.dataset == 'touch_hard' or args.dataset == 'feeling':
        ...
        elif args.dataset == 'feeling':
            train_dataset = FeelingLabel(data_folder,  transform=train_transform, mode='train')
            val_dataset = FeelingLabel(data_folder, transform=train_transform, mode='test')
        else:
...
def set_model(args):
    ...
    ckpt_init = torch.load(args.model_path)
    ckpt = ckpt_init['state_dict']   
    new_ckpt = {}
    for k,v in ckpt.items():
        key = k.replace("module.", "")
        # if "vision_model" in key:
        #     key = key.replace("base_model.model.", "")
        new_ckpt[key] = copy.deepcopy(v)
   

    msg = model.load_state_dict(new_ckpt, strict=False)
    print(msg)
    if args.dataset == "feeling":
        # classifier = LinearClassifierResNet(model.projection_dim, args.n_label)
        classifier = LinearClassifierResNet(model.touch_model.config.hidden_size*4, args.n_label)
    else:
        classifier = LinearClassifierResNet(model.touch_model.config.hidden_size, args.n_label)
...
def train(epoch, train_loader, model, classifier, criterion, optimizer, opt):
    ...
    for idx, (input, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            if input.shape[1] <= 3:
                #feat_touch = model.encode_touch(input)
                feat_touch = model.encode_touch_rm_proj(input)
            else:
                gelsightA_before_feat, gelsightA_after_feat, gelsightB_before_feat, gelsightB_after_feat = model.encode_feeling_touch_rm_proj(input)
                #print(gelsightA_after_feat.shape,gelsightA_after_feat.shape,gelsightB_before_feat.shape,gelsightB_after_feat.shape)
                
                feat_touch = torch.concatenate((gelsightA_before_feat,gelsightA_after_feat,gelsightB_before_feat,gelsightB_after_feat),dim=1)
                #print(feat_touch.shape)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if test_acc > best_acc1:
            ...
            save_name = 'best_{epoch}.pth'.format(epoch=epoch)
